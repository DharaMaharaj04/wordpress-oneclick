...
function custom_limit_login_attempts($user_login) {
    $max_attempts = 3; // Max attempts
    $lockout_time = 15 * MINUTE_IN_SECONDS; // Lockout time in seconds (15 minutes)
    
    $attempts = (int) get_option('login_attempts_' . $user_login, 0);
    
    if ($attempts >= $max_attempts) {
        $lockout = get_option('lockout_time_' . $user_login);
        if (time() < $lockout) {
           
            $remaining_time = ceil(($lockout - time()) / 60);
            wp_die("Too many failed login attempts. Please try again in $remaining_time minutes.");
        } else {
            
            delete_option('login_attempts_' . $user_login);
            delete_option('lockout_time_' . $user_login);
        }
    }
}

add_action('wp_authenticate', 'custom_limit_login_attempts');

function record_failed_login_attempts($username) {
    $attempts = (int) get_option('login_attempts_' . $username, 0);
    $attempts++;
    
    update_option('login_attempts_' . $username, $attempts);
    if ($attempts >= 3) {
        update_option('lockout_time_' . $username, time() + (15 * MINUTE_IN_SECONDS));
    }
}

add_action('wp_login_failed', 'record_failed_login_attempts');
...
function display_login_attempts_message() {
    if (isset($_POST['log'])) {
        $username = sanitize_user($_POST['log']);
        $attempts = (int) get_option('login_attempts_' . $username, 0);
        if ($attempts > 0) {
            echo '<div class="login-attempts-message" style="color: red;">';
            echo 'Login attempts made: ' . $attempts;
            echo '</div>';
        }
    }
}
add_action('login_form', 'display_login_attempts_message');
